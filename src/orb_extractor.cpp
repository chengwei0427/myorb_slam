...
void ORBextractor::computeKeyPointsQuadTree(std::vector<std::vector<cv::KeyPoint>>& all_keypoints)
{
	...
	for (int level = 0; level < levels_num_; ++level)
	{
		...
		//待检测区域划分为格子的行列数
		const int ncols = width / border_width;
		const int nrows = height / border_width;
		//重新计算每个格子大小
		const int width_cell = ceil(width / ncols);
		const int height_cell = ceil(height / nrows);
		//在每个格子进行fast特征检测
...
void ORBextractor::operator()(cv::InputArray image_array, std::vector<cv::KeyPoint>& output_keypoints,
